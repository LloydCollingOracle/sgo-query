...
public class QueryStringBuilderTest {
    ...
    /**
     * Tests that the GROUP_CONCAT aggregate function is inserted properly and the column is detected as a grouping
     */
    public void testGroupConcatQuery() {
        Query query = new Query().setObjectClassName("org.sgodden.example.Site");
        query.addColumn("name");
        query.addColumn("code");
        query.addColumn("allTms.person.name", AggregateFunction.GROUP_CONCAT);
        
        Session s = createMock(Session.class);
        org.hibernate.Query q = createNiceMock(org.hibernate.Query.class);

        expect(s.createQuery(eq("SELECT obj.id, obj.name, obj.code, GROUP_CONCAT(allTmsperson.name) " +
        		"FROM org.sgodden.example.Site AS obj LEFT OUTER JOIN obj.allTms AS allTms LEFT OUTER JOIN allTms.person AS allTmsperson " +
        		"GROUP BY obj.id , obj.name, obj.code ORDER BY 2, 3, 4, 1"))).andReturn(q);
    ...
    /**
     * Tests that the GROUP_CONCAT_DISTINCT aggregate function is inserted properly and the column is detected as a grouping
     */
    public void testGroupConcatDistinctQuery() {
        Query query = new Query().setObjectClassName("org.sgodden.example.Site");
        query.addColumn("name");
        query.addColumn("code");
        query.addColumn("allTms.person.name", AggregateFunction.GROUP_CONCAT_DISTINCT);
        
        Session s = createMock(Session.class);
        org.hibernate.Query q = createNiceMock(org.hibernate.Query.class);

        expect(s.createQuery(eq("SELECT obj.id, obj.name, obj.code, GROUP_CONCAT_DISTINCT(allTmsperson.name) " +
        		"FROM org.sgodden.example.Site AS obj LEFT OUTER JOIN obj.allTms AS allTms LEFT OUTER JOIN allTms.person AS allTmsperson " +
        		"GROUP BY obj.id , obj.name, obj.code ORDER BY 2, 3, 4, 1"))).andReturn(q);
    ...
    /**
     * Ensures that if we have multiple GROUP_CONCAT that all appear correct and not used for grouping
     */
    public void testMultipleGroupConcatQuery() {
        Query query = new Query().setObjectClassName("org.sgodden.example.Site");
        query.addColumn("name");
        query.addColumn("code");
        query.addColumn("allTms.person.name", AggregateFunction.GROUP_CONCAT);
        query.addColumn("allTms.person.email", AggregateFunction.GROUP_CONCAT);
        
        Session s = createMock(Session.class);
        org.hibernate.Query q = createNiceMock(org.hibernate.Query.class);

        expect(s.createQuery(eq("SELECT obj.id, obj.name, obj.code, GROUP_CONCAT(allTmsperson.name), GROUP_CONCAT(allTmsperson.email) " +
        		"FROM org.sgodden.example.Site AS obj LEFT OUTER JOIN obj.allTms AS allTms LEFT OUTER JOIN allTms.person AS allTmsperson " +
        		"GROUP BY obj.id , obj.name, obj.code ORDER BY 2, 3, 4, 5, 1"))).andReturn(q);
    ...
    /**
     * Ensures that if we have multiple GROUP_CONCAT_DISTINCT that all appear correct and not used for grouping
     */
    public void testMultipleGroupConcatDistinctQuery() {
        Query query = new Query().setObjectClassName("org.sgodden.example.Site");
        query.addColumn("name");
        query.addColumn("code");
        query.addColumn("allTms.person.name", AggregateFunction.GROUP_CONCAT_DISTINCT);
        query.addColumn("allTms.person.email", AggregateFunction.GROUP_CONCAT_DISTINCT);
        
        Session s = createMock(Session.class);
        org.hibernate.Query q = createNiceMock(org.hibernate.Query.class);

        expect(s.createQuery(eq("SELECT obj.id, obj.name, obj.code, GROUP_CONCAT_DISTINCT(allTmsperson.name), GROUP_CONCAT_DISTINCT(allTmsperson.email) " +
        		"FROM org.sgodden.example.Site AS obj LEFT OUTER JOIN obj.allTms AS allTms LEFT OUTER JOIN allTms.person AS allTmsperson " +
        		"GROUP BY obj.id , obj.name, obj.code ORDER BY 2, 3, 4, 5, 1"))).andReturn(q);
